...
function renderLibrary() {
  ...
  el.innerHTML = songs.slice(0, 8).map(s => `
  ...
  el.querySelectorAll('.lib-item').forEach(li => {
    li.addEventListener('click', () => playSongById(parseInt(li.dataset.sid)));
  });
}
  ...
  return `
    <div class="music-card ${isCurrent ? 'is-playing' : ''}" data-sid="${s.id}" style="animation-delay:${animDelay}ms">
      <div class="card-thumb">
        ${s.img
          ? `<img src="${esc(s.img)}" alt="${esc(s.title)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
          : ''
        }
...
function renderGrids() {
  ...
  document.querySelectorAll('.music-card[data-sid]').forEach(card => {
    card.addEventListener('click', () => playSongById(parseInt(card.dataset.sid)));
  });
}
...
function playSongById(sid) {
  ...
  const newIdx = list.findIndex(s => s.id === sid);
  if (newIdx === -1) return;
  playlist = [...list];
  idx = newIdx;
  loadSong(true);
...
bindEl('heartBtn', () => {
  ...
  if (hb) hb.classList.toggle('liked', liked.has(song.id));
});
...
if (volIcon) {
  volIcon.addEventListener('click', () => {
    if (audio.volume > 0) {
      audio.volume = 0; if (volSlider) volSlider.value = 0; updateVolIcon(0);
    } else {
      audio.volume = lastVol / 100; if (volSlider) volSlider.value = lastVol; updateVolIcon(lastVol);
    }
  });
}
...
function renderLyricsContent(song) {
  ...
  if (!content) return;
  if (!song) {
    content.innerHTML = `<div class="lyrics-empty"><i class="fas fa-microphone-slash"></i><p>Bir şarkı seçerek<br>sözleri görüntüle.</p></div>`;
    return;
  }
  if (!song.lyrics || song.lyrics.length === 0) {
    content.innerHTML = `<div class="lyrics-empty"><i class="fas fa-music"></i><p>Bu şarkı için henüz<br>söz eklenmemiş.</p></div>`;
    return;
  }
        ...
        <div class="verse-lines">${
          verse.lines.map(line =>
            line.trim()
              ...
              : `<div class="lyric-line" style="height:6px;"></div>`
          ).join('')
        }</div>
      </div>
    `).join('')
...
// ─── INIT ─────────────────────────────────────────
function init() {
  // Admin'den değişiklikler varsa yeniden yükle
